...
const previewBatch = useAnimationFrameBatch<{ columnKey: AccountTableColumnKey; width: number }>(
...
function previewResize(rawEvent: unknown): void {
  ...
  event.preventDefault();
  const width = clampAccountTableColumnWidth(
    props.columnKey,
...
<template>
  <button
    ...
    :aria-valuemin="MIN_ACCOUNT_TABLE_COLUMN_WIDTHS[columnKey]"
    :aria-valuemax="MAX_ACCOUNT_TABLE_COLUMN_WIDTH"
    :aria-valuenow="width"
