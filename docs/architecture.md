...
经营数据报表通过独立的只读 `get_revenue_analytics_report` command 按需加载，不增加收益页初始化
请求。该 command 只返回不含凭据的基础设施聚合 DTO：自然周与星期仍按 `serviceDate` 归属完整预约
工时，24 小时热力按已完成预约的真实起止时间拆分重叠分钟，顾客贡献沿用收益联系人归一规则。报表
范围切换或数据修订后，前端关闭旧弹层并拒绝把 stale 响应用于当前分析。
